      ...
      >
              ...
              <div className="slider__label-start">
                <div className="slider__label-start-inner">
                  <span className="aria-label">
                    {_globals._components._slider.labelStart} {scaleStepPrefix}
                    {_scaleStart}
                    {scaleStepSuffix}
                  </span>
                  {labelStart}
            ...
            {labelEnd && (
              <div className="slider__label-end">
                <div className="slider__label-end-inner">
                  <span className="aria-label">
                    {_globals._components._slider.labelEnd} {scaleStepPrefix}
                    {_scaleEnd}
                    {scaleStepSuffix}
                  </span>
                  {labelEnd}
            ...
            _items.map(({ index, value }) => {
              return (
                <div
                  ...
                  aria-hidden="true"
                  style={{
                    left: `${calculatePercentFromIndex(index)}%`,
                    backgroundColor: index === selectedIndex ? "#02D2A0" : "",
              ...
              getCorrectAnswers().map((correctAnswer) => {
                return (
                  <div
                    ...
                    key={correctAnswer}
                    style={{
                      left: `${calculatePercentFromIndex(
                        getIndexFromValue(correctAnswer)
                      )}%`,
                    }}
                  >
                    {_showNumber &&
        ...
        >
          <div className="slider__item-input-track">
            <div
              className="slider__item-input-fill"
              style={{ width: `${selectedWidth}%` }}
            />
          </div>

          <input
            className="slider__item-input js-slider-item-input"
            type="range"
            role="slider"
            value={selectedValue}
            min={_scaleStart}
